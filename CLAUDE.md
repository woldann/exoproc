...
- **`RemoteCallableMemoryAccessor`** (`callable-accessor.ts`) — creates a remote thread via `CreateRemoteThread` for each call. `call()` uses a 5ms `WaitForSingleObject` + `setImmediate` polling loop (not blocking wait) so the event loop can process I/O between iterations. Do **not** use `callAsync` on `WaitForSingleObject` — causes Wine GC deadlock.
...
`callAsync` uses `QueueUserWorkItem` + `JSCallback(threadsafe)`, which requires JIT-compiled code to run in a Windows thread pool thread. This was previously **blocked by security policies on GitHub's `windows-latest` runners**, so `callAsync`/`Handle.wait(timeoutMs > asyncCallOverheadMs)` tests were guarded with `process.env.CI ? test.skip : test`. Now that CI runs the suite under Wine on `ubuntu-latest` (see the `test` job in `.github/workflows/ci.yml`) instead of a real Windows-hosted runner, that OS-level block no longer applies and these tests run unguarded in CI too — tolerate ordinary Wine flakiness with try/catch around the assertion (see `tests/winapi/winapi.test.ts`'s `wait(100)` test) rather than skipping.

#### `callAsync` hangs indefinitely on **real** Windows (confirmed, not just Wine/CI flakiness)

Unlike the CI/Wine-only issues above, this reproduces on real Windows hardware: `Native.calibrateAsyncOverhead()` (and anything else that hits `callAsync` for the first time, e.g. `Handle.wait(timeoutMs > asyncCallOverheadMs)`) can hang **forever** — not time out, not throw, the process just sits there pegging exactly one CPU core, and even `bun:test`'s own test-level timeout never fires (meaning the JS event loop itself is wedged, not merely a background thread waiting).

Root-caused via targeted diagnostic logging (temporarily added around `compileSignatures`/`cjitopen` and around the `QueueUserWorkItem` call + JSCallback body in `cfunction.ts`, then removed once confirmed) to the exact hand-off point: the lazy JIT compile of the ~363-signature `callAsync` thread-pool-proc pool completes fine (comparable timing to Wine, ruling out "JIT compile is just slow"), and `QueueUserWorkItem` itself returns success (the work genuinely gets queued) — but the dispatched worker thread's call into the threadsafe `JSCallback` never makes it back to JS at all. Bun's own docs confirm this is expected to be unreliable:

> "Thread-safe callbacks work best when run from another thread that is running JavaScript code, that is, a Worker. A future version of Bun will enable them to be called from any thread, such as new threads spawned by your native library that Bun is not aware of."

`QueueUserWorkItem`'s worker threads are exactly that second case — plain native NT thread-pool threads Bun has no knowledge of, not a Bun `Worker`. This is an upstream Bun engine limitation, not a bug fixable from this repo's code, and it's apparently timing/implementation-dependent enough that Wine's own thread-pool emulation doesn't hit it (every Wine run in this repo's history has passed this exact test).

**Mitigation applied:** `getQueueWorkItemFn`'s C wrapper now propagates `QueueUserWorkItem`'s `BOOL` return value instead of discarding it, and `makeCallAsync` rejects immediately (freeing the orphaned buffer) if it's 0 — this at least converts a _submission_ failure into an immediate rejected promise instead of a silent hang. It does **not** fix the confirmed real-Windows case above, where `QueueUserWorkItem` itself reports success; there's currently no way to detect or bound that failure mode from JS once the work has been handed off; if you're testing on real Windows and hit this, you'll need to kill the process. Fixing this properly would mean not dispatching through a Bun-unaware native thread at all (e.g. routing the blocking call through a real Bun `Worker` instead of `QueueUserWorkItem`) — a real architectural change, deliberately deferred rather than attempted blind since it can't be verified from this (Linux + Wine) development environment.
...
The first two remain timing-sensitive races in Wine's own emulation rather than logic bugs in this repo. If you hit one of _those_, just re-run — it isn't a regression from your change unless you can reproduce it going from 0 crashes to consistently crashing on the same test file in isolation on your branch specifically (verified against an unmodified tree first).
