...
- **`RemoteCallableMemoryAccessor.call()` (`CreateRemoteThread` into a _different_ process, one fresh thread per call) reliably delivers every argument** — that part of the mechanism is solid and is what `RemoteCallableMemoryAccessor`'s entire calling convention depends on. But if the _called code itself_ performs a real WinAPI/CRT call (`CallRedirectorAccessor.alloc()` → `VirtualAlloc`, `IndirectCallRedirectorAccessor.alloc()` → `malloc`, `FileTransferReadAccessor`/`FileTransferWriteAccessor` → `fopen`, `isModuleLoadedInProcess()` → `GetModuleHandleExA`, or injected `cmachinecode()` that itself calls `VirtualAlloc`), that call can crash Wine outright (`wine: Unhandled page fault... starting debugger`) or the whole remote thread wedges — the same freshly-created-thread bug, just hit remotely instead of locally. This was masked for a long time because the old `NamedPipeCallableAccessor` only ever used one `CreateRemoteThread` call, to start a persistent loop thread; every actual call after that ran on that same already-live thread, never a fresh one. Once `NamedPipeCallableAccessor` was removed, chains that route real WinAPI calls through a bare `RemoteCallableMemoryAccessor` (`CallRedirectorAccessor`/`IndirectCallRedirectorAccessor`/`FileTransferRead|WriteAccessor`/`isModuleLoadedInProcess`/the default pid-based `IndirectCallableAccessor`) started failing/timing out/crashing Wine in CI — including for a **single** call (`isModuleLoadedInProcess`'s `GetModuleHandleExA(FROM_ADDRESS)` check against a deliberately bogus address is one real syscall on one fresh thread, and that alone crashed Wine). Don't assume "just one call" is automatically safe — the risk is any real WinAPI/CRT call touching a freshly-created thread, not the call count.
