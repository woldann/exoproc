...
description: "Exoproc is a TypeScript/Bun toolkit for cross-process instrumentation on Windows x64. It can read and write target memory, run calls on an..."
---
...
Remote work is more fragile than ordinary application code: a bad address, type, signature, or thread choice can crash the target. Continue with [NThread](/en/docs/nthread/overview) and [troubleshooting](/en/docs/troubleshooting) before using a non-test target.
