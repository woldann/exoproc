...
const content = {
  tr: {
    ...
    secondaryAction: { text: 'NThread tasarımı', href: '/tr/docs/nthread/overview' },
    features: [
      {
        title: 'NThread',
    ...
    disclaimer: 'Bu proje yalnızca yetkili güvenlik araştırması, hata ayıklama ve eğitim amaçlı kullanılmalıdır.',
  },
  en: {
    title: 'Exoproc',
    ...
    secondaryAction: { text: 'NThread design', href: '/en/docs/nthread/overview' },
    features: [
      {
        title: 'NThread',
    ...
    disclaimer: 'Use Exoproc only for authorized security research, debugging, and education.',
  },
} as const;

export default async function HomePage(props: PageProps<'/[lang]'>) {
  const { lang } = await props.params;
  const c = content[lang as keyof typeof content] ?? content[i18n.defaultLanguage as keyof typeof content];
    ...
    <div className="flex flex-1 flex-col items-center px-6 py-16">
      <div className="flex max-w-2xl flex-col items-center text-center">
        ...
        <div className="mt-6 flex gap-3">
          ...
          <Link href={c.secondaryAction.href} className="rounded-lg border px-4 py-2 font-medium">
      ...
      <div className="mt-16 grid max-w-4xl gap-6 sm:grid-cols-3">
        {c.features.map((feature) => (
          <div key={feature.title} className="rounded-lg border p-4">
            ...
            <p className="mt-2 text-sm text-fd-muted-foreground">{feature.details}</p>
          </div>
        ))}
      </div>
