...
import { CommandService, CommandsRegistry } from '@/platform/commands/common/commands';
import { KeybindingService, KeybindingsRegistry } from '@/platform/keybinding/common/keybinding';
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const off = debug.onDidChangeThread((snapshot) => pushed.push(snapshot));

        const proc = await machine.createDemoProcess();
        add(`process pid=${proc.pid} image=${proc.image} threads=${proc.threads.length} mappings=${proc.mappings.length}`);
        ...
        add(`step reason=${step?.reason} mnemonic=${step?.instruction.mnemonic}`);
        add(`rip moved=${after !== undefined && after.registers.RIP !== before}`);
        ...
        add(`memory bytes=${bytes.byteLength} isU8=${bytes instanceof Uint8Array}`);
        ...
        const first = await Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('scan.first timed out (60s)')), 60_000),
          ),
        ]);
        add(`scan took=${Math.round(performance.now() - started)}ms`);
        add(`scan first total=${first.total} page=${first.results.length} truncated=${first.truncated}`);
        const narrowed = await scan.next(proc.pid, { compare: 'unchanged' });
        add(`scan next total=${narrowed.total} narrowed=${narrowed.total <= first.total}`);
        if (narrowed.results.length > 0) {
          const probe = narrowed.results[0].address;
          add(`scan readValue type=${typeof (await scan.readValue(proc.pid, probe))}`);
        }

        // Worst realistic case: unknown-value scan across every mapping.
        const wideStart = performance.now();
        const wide = await scan.first(proc.pid, { type: 'i32', compare: 'unknown' });
        add(`scan wide took=${Math.round(performance.now() - wideStart)}ms total=${wide.total} truncated=${wide.truncated}`);

        // -------------------------------------------------- workspace + fs
        const { workspace, fs } = window.exoproc;

        const emptyInfo = await workspace.bind({ type: 'empty' });
        add(`workspace bind empty root=${emptyInfo.rootName} label=${emptyInfo.sourceLabel}`);

        await fs.writeFile('/hello.txt', new TextEncoder().encode('persisted?'), {
          create: true,
          overwrite: true,
        });
        const readBack = new TextDecoder().decode(await fs.readFile('/hello.txt'));
        add(`fs write+read roundtrip=${readBack === 'persisted?'}`);

        const listing = (await fs.readDirectory('/')).map(([name]) => name);
        add(`fs readDirectory sees written file=${listing.includes('hello.txt')}`);
        ...
        const offFiles = fileService.onDidChangeFile((changes) => {
          for (const change of changes) fileEvents.push(`${change.kind}:${change.path}`);
        });
        ...
        const dirEntries = (await fileService.readDirectory('/notes')).map(([n]) => n);
        add(`IFileService readDirectory=${dirEntries.join(',')}`);

        const info = fileService.getWorkspaceInfo();
        add(`IFileService getWorkspaceInfo cached=${info?.sourceLabel === 'empty'}`);

        await fileService.rename('/notes/todo.md', '/notes/todo-renamed.md', {
          overwrite: false,
        });
        const renamedText = await fileService.readFileText('/notes/todo-renamed.md');
        ...
        await fileService.bindWorkspace({ type: 'empty' });
        add(`IFileService onDidChangeWorkspace fired=${workspaceEvents.length === 1}`);
        ...
        add(`terminal create id=${typeof session.id === 'string' && session.id.length > 0}`);
        ...
        add(`terminal banner received=${terminalOutput.includes('Microsoft Windows')}`);
        ...
        add(`terminal echo output=${terminalOutput.includes('hello-from-terminal')}`);
