...
import { getNamedPipeMachineCode } from './named-pipe-machinecode.js';
...
import { type CFunction, parseCallResult } from './cfunction.js';
...
export class NamedPipeCallableAccessor extends InittableMiddlewareAccessor {
  private pipeServer: net.Server | null = null;
  private pipeSocket: net.Socket | null = null;
  private pipeName!: string;
  private pipeAddress!: AddressLike;
  private scAddress = 0;
  ...
  override async call(func: CFunction, ...args: any[]): Promise<CCallResult> {
    if (this.scAddress && Number(resolveAddress(func)) === this.scAddress) {
      return this.backend.call(func, ...args);
    }
    return super.call(func, ...args);
  }
  ...
  protected async onInit(): Promise<void> {
    this.pipeName = String.raw`\\.\pipe\Exoproc_${randomUUID()}`;
    ...
    await new Promise<void>((resolve, reject) => {
      this.pipeServer!.listen(this.pipeName, () => resolve());
      this.pipeServer!.on('error', reject);
    });

    try {
      this.pipeAddress = await this.root.alloc(this.pipeName.length + 1);
      await this.root.write(
        this.pipeAddress,
        Buffer.from(this.pipeName + '\0'),
      );
      const pipeNameAddrVal = Number(resolveAddress(this.pipeAddress));

      this.scAddress = await this.root.machineCode(getNamedPipeMachineCode());
      const remoteLoop = getNamedPipeMachineCode().cloneForAddress(
        this.scAddress,
      );
      // Don't await — the machineCode blocks in the remote process reading from the pipe.
      // this.root must be a BootstrapHostAccessor (or similar) so that during init it routes
      // through this.backend directly (isInitializing guard skips re-init on pipeAccessor),
      // and after init it routes through the full middleware chain.
      this.root.call(remoteLoop as any, pipeNameAddrVal);
        ...
        new Promise<void>((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(
                  'Timeout waiting for remote thread named pipe connection.',
                ),
              ),
            10000,
          ),
      ...
      await this.deinit().catch(() => {});
      throw err;
    }
  }

  protected override async onDeinit(): Promise<void> {
    if (this.pipeSocket && !this.pipeSocket.destroyed) {
      try {
        const exitBuf = Buffer.alloc(144); // zeroed targetFunc tells dispatcher thread to exit
        await new Promise<void>((resolve) => {
          this.pipeSocket!.write(exitBuf, () => resolve());
        });
      } catch {
        /* ignore errors on shutdown write */
      }
    }
    ...
    if (this.pipeAddress) {
      await this.root.free(this.pipeAddress).catch(() => {});
      this.pipeAddress = 0;
    }

    if (this.scAddress) {
      await this.root.free(this.scAddress).catch(() => {});
      this.scAddress = 0;
    }
  ...
  override async callAfterInit(
    func: CFunction,
    args: any[] = [],
  ): Promise<CCallResult> {
    return new Promise((resolve, reject) => {
      this.callQueue.push({ func, args, resolve, reject });
      this.processQueue();
    });
  }
  ...
  private async processQueue() {
    ...
    try {
      while (this.callQueue.length > 0) {
        ...
        try {
          const res = await this.executeCall(item.func, item.args);
          item.resolve(res);
        } catch (err) {
          item.reject(err);
        }
  ...
  private async executeCall(
    ...
    if (!this.pipeSocket) {
      throw new Error(
        'NamedPipeCallableAccessor: Pipe socket is not connected.',
      );
    }
    const socket = this.pipeSocket;
    const returns = func.returns;
    const targetFuncAddr = Number(resolveAddress(func.ptr));
    const argAddrs = args.map((arg) => Number(resolveAddress(arg)));

    const reqBuf = Buffer.alloc(144);
    reqBuf.writeBigUInt64LE(BigInt(targetFuncAddr), 0);
    reqBuf.writeUInt32LE(argAddrs.length, 8);
    for (let i = 0; i < 16; i++) {
      const val = i < argAddrs.length ? BigInt(argAddrs[i]!) : 0n;
      reqBuf.writeBigUInt64LE(val, 16 + i * 8);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const onData = (chunk: Buffer) => {
        ...
        if (accum.length >= 8) {
          ...
          const result = this.parseResult(accum.subarray(0, 8), returns);
          resolve(result);
        }
      };
  ...
  private parseResult(buffer: Buffer, type: any) {
    if (buffer.length === 0) return undefined;
    return parseCallResult(buffer.readBigUInt64LE(0), type);
  }
}
